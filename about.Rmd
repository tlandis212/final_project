...
```{r}
  ...
  ggplot(ms_six, aes(x = treatment)) + 
  geom_bar(aes(y = (..count..)/(..count..))) +
  ...
  labs(x = "Voting Method",
       y = "Percent of Voters",
       title = "Voting Method in the 2012 General Election",
       subtitle = "18-year-old Voters from Alamance County, NC",
       fill = "Voted Absentee in\n2012 General Election")

ms_six_b <- ms_six %>% 
  mutate(treatment_2014 = ifelse(treatment == TRUE & gelection_2014 != "No Vote", TRUE, FALSE),
         control_2014 = ifelse(treatment == FALSE & gelection_2014 != "No Vote", TRUE, FALSE))
  
ms_six_b %>% 
  count(control_2014)

ms_six_b %>% 
  count(treatment_2014)


ggplot(ms_six_b, aes(x = treatment_voted_2014)) + 
  geom_bar(aes(y = (..count..)/(..count..))) +
  scale_y_continuous(labels=scales::percent) +
    geom_text(aes( label = scales::percent((..count..)/(..count..)),
  y= (..count..)/(..count..)), stat= "count", vjust = -1) +
  theme_classic() +
  labs(x = "Voting Method",
       y = "Percent of Voters",
...
ggplot(ms_six_b, aes(x = gelection_2016, fill = treatment)) + 
