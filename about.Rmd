...
ms_six_b <- alamance_small %>% 
  mutate(treatment = ifelse(gelection_2012 == "Absentee", 1, 0),
         outcome_2012 = ifelse(gelection_2012 != "No Vote", 1, 0),
         outcome_2014 = ifelse(gelection_2014 != "No Vote", 1, 0),
         outcome_2016 = ifelse(gelection_2016 != "No Vote", 1, 0))

ms_six_b %>% 
  select(treatment, outcome_2012, outcome_2014, outcome_2016) %>% 
  group_by(treatment) %>% 
  aggregate()
  ...
  # Sum by a certain amount. Save as a new dataframe
...
# X axis as year. Subsequent years, % of 2012. 
```
